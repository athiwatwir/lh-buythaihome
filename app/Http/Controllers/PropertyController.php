...
{
    use LoadsRecommendedProperties;
    ...
    public function index(Request $request, LoveThaiHomeService $api): View
    {
        ...
        $isRecommendFilter = $this->isRecommendFilterRequest($request);
        ...
        try {
            if ($isSearch) {
                ...
                $response = $api->properties([
                    ...
                    'agent_id' => $agentId,
                    'user_id' => $userId,
                    'is_recommend' => $isRecommendFilter ? 'Y' : null,
                    'page' => $page,
                    'per_page' => $perPage,
                ]);
        ...
        return view('pages.properties.index', [
            ...
            'isRecommendFilter' => $isRecommendFilter,
        ]);
    }
    ...
    public function show(string $property, LoveThaiHomeService $api): View
    {
        ...
        $recommendedProperties = $this->loadRecommendedProperties($api, 12)
            ->reject(fn(PropertyData $item) => $item->id === $detail->id)
            ->values();
    ...
    public function showByCode(string $propertyTypeCode, string $propertyCode, LoveThaiHomeService $api): View
    {
        try {
            $detail = $api->property($propertyCode);
        } catch (LoveThaiHomeApiException $exception) {
            Log::warning('Failed to load property detail from API.', [
                'message' => $exception->getMessage(),
                'status' => $exception->statusCode,
    ...
    private function isRecommendFilterRequest(Request $request): bool
    {
        if ($request->boolean('is_recommend')) {
            return true;
        }

        $value = strtoupper($request->string('is_recommend')->toString());
