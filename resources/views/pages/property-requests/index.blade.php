...
<div class="bg-gray-50 antialiased">
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 items-start gap-8 xl:grid-cols-12 xl:gap-10">
            <aside class="order-2 space-y-6 xl:order-1 xl:col-span-5 xl:sticky xl:top-24">
                <div class="overflow-hidden rounded-2xl border border-blue-100 bg-linear-to-br from-blue-50 to-white p-4 shadow-sm md:p-5">
                    <p class="text-xs font-semibold uppercase tracking-wide text-blue-600">ทำไมต้องฝากขายกับเรา</p>
                    <h2 class="heading-font mt-1 text-xl font-bold text-blue-900 md:text-2xl">ปรึกษาฟรี ไม่มีค่าใช้จ่าย</h2>
                    <p class="mt-2 text-sm leading-relaxed text-gray-600">
                        ทีมงานมืออาชีพพร้อมดูแลตั้งแต่ประเมินราคา โปรโมทออนไลน์ จนปิดการขาย
                    </p>
                </div>

                <x-consignment-benefits-video />

                <x-consignment-benefits-list compact />
            </aside>

            <div class="order-1 xl:order-2 xl:col-span-7">
                <form method="POST" action="{{ route('property-requests.store') }}" enctype="multipart/form-data" class="space-y-6 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm md:p-8">
                    @csrf
                    ...
                    <section>
                        ...
                        <div class="mt-4 space-y-4">
                            <div>
                                ...
                                <select id="asset_type_id" name="asset_type_id" required class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <div>
                                    <label for="price_amount" class="mb-1 block text-sm font-medium text-gray-700">ราคาขาย (บาท) <span class="text-red-500">*</span></label>
                                    <input type="number" id="price_amount" name="price_amount" value="{{ old('price_amount') }}" min="0" step="1" required placeholder="เช่น 3500000" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
                                    @error('price_amount')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    ...
                                    <input type="number" id="price_per_wah" name="price_per_wah" value="{{ old('price_per_wah') }}" min="0" step="1" placeholder="เช่น 50000" class="w-full rounded-xl border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
                                    @error('price_per_wah')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div x-data="{
                                ...
                                <label for="images" class="mb-1 block text-sm font-medium text-gray-700">รูปภาพทรัพย์</label>
                                <input type="file" id="images" name="images[]" accept="image/jpeg,image/jpg,image/png,image/webp" multiple @change="updatePreviews($event)" class="block w-full cursor-pointer rounded-xl border border-gray-300 bg-gray-50 px-3 py-2.5 text-sm text-gray-700 file:mr-3 file:rounded-lg file:border-0 file:bg-blue-700 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-white hover:file:bg-blue-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
                                <p class="mt-1 text-xs text-gray-500">อัปโหลดได้สูงสุด 10 รูป (JPG, PNG, WEBP ไม่เกิน 10 MB ต่อรูป)</p>
                                @error('images')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                                @error('images.*')
                                ...
                                <div x-show="previews.length > 0" x-cloak class="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4">
                                    <template x-for="preview in previews" :key="preview.url">
                                        <div class="overflow-hidden rounded-lg border border-gray-200 bg-gray-50">
                                            <img :src="preview.url" :alt="preview.name" class="aspect-square h-full w-full object-cover">
                                        </div>
                                    </template>
                                </div>
                            </div>
                    ...
                    <div class="border-t border-gray-100 pt-6">
                        <label class="flex cursor-pointer items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3">
                            <input type="checkbox" name="isreqconsult" value="1" class="mt-1 h-4 w-4 rounded border-gray-300 text-blue-700 focus:ring-blue-500" @checked(session()->has('errors') ? (bool) old('isreqconsult') : true)
                            >
                    ...
                    <div class="pt-2">
                        <button type="submit" class="heading-font w-full rounded-xl bg-blue-700 px-6 py-3.5 text-base font-bold text-white shadow-md transition hover:bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                            ส่งข้อมูลฝากขาย
                        </button>
                        <p class="mt-3 text-center text-xs text-gray-500">
                            ข้อมูลของคุณจะถูกส่งไปยังทีมงาน Buythaihome เพื่อดำเนินการต่อ
                        </p>
                    </div>
                </form>
            </div>
